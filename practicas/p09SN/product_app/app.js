...
function init() {
    ...
    // SE LISTAN T9ODOS LOS PRODUCTOS
    listarProductos();
}
...
function agregarProducto(e) {
    ...
    // VALIDAR EL PRODUCTO
    let errores = validarProducto(finalJSON);
    if (errores.length > 0) {
      // MOSTRAR ERRORES EN LA BARRA DE ESTADO
      let template_bar = '';
      template_bar += '<ul>';
      for (let i = 0; i < errores.length; i++) {
        template_bar += '<li style="list-style: none;">' + errores[i] + '</li>';
      }
      template_bar += '</ul>';
      document.getElementById("product-result").className = "card my-4 d-block";
      document.getElementById("container").innerHTML = template_bar;
